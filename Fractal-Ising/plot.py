...
color = [ (0,0,0), (0.72,0,0),(0.8,0,0.42), (0.9,0,0.7) ]
markercolor = [ (0,0,0), (0.72*0.8,0,0),(0.8*0.8,0,0.42*0.8), (0.9*0.8,0,0.7*0.8) ]


for i in range(len(Latticesizes)):
    print(Latticesizes[i])
    folderpath = 'L=' + str(Latticesizes[i])
    f = h5py.File('%s/Fractal.out.h5' %folderpath,'r')    
...
######## Fetch Data ########
    for q in range(Npoints):
        ...
        g[q,:] = np.array(f['/simulation/results/%s/g/mean/value' %q] )
...
######## Plot ########
    ...
    plt.figure('fractalOP', figsize=(8,5))
    for q in range(Npoints):
        y[q] = np.float64(f['/simulation/results/%s/fractalOP/mean/value'%q]) * c[q]
        dy[q] = np.float64(f['/simulation/results/%s/fractalOP/mean/error'%q]) * c[q]
    plt.xlabel('T')
    plt.errorbar(T,y,dy,marker='.', linewidth = 0.8,label = L)
    plt.ylabel('fractalOP')


    plt.figure('fractalOP_Susc', figsize=(8,5))
    for q in range(Npoints):
        y[q] = np.float64(f['/simulation/results/%s/fractalOP_Susc/mean/value'%q])  * c[q] *lat_sites / T[q]
        dy[q] = np.float64(f['/simulation/results/%s/fractalOP_Susc/mean/error'%q]) * c[q] *lat_sites / T[q]
    plt.xlabel('T')
    plt.errorbar(T, y, dy, marker = '.', linewidth=0.8,label = L)
    plt.ylabel('fractalOP_susc')



    plt.figure('magFM', figsize=(8,5))
    for q in range(Npoints):
        y[q] = np.float64(f['/simulation/results/%s/magFM/mean/value'%q]) * c[q]
        dy[q] = np.float64(f['/simulation/results/%s/magFM/mean/error'%q]) * c[q]
    plt.xlabel('T')
    plt.errorbar(T,y,dy,marker='.', linewidth = 0.8,label = L)
    plt.ylabel('magFM')


    plt.figure('magFM_susc', figsize=(8,5))
    for q in range(Npoints):
        y[q] = np.float64(f['/simulation/results/%s/magFM_Susc/mean/value'%q])  *lat_sites / T[q]
        dy[q] = np.float64(f['/simulation/results/%s/magFM_Susc/mean/error'%q]) *lat_sites / T[q]
    plt.xlabel('T')
    plt.errorbar(T, y, dy, marker = '.', linewidth=0.8,label = L)
    plt.ylabel('magFM_susc')
    ...
    plt.figure('fractalOP_BC', figsize=(8,5))
    for q in range(Npoints):
        y[q] = 1 - (1/3) *np.float64(f['/simulation/results/%s/fractalOP_Kurt/mean/value'%q]) 
        dy[q] = (1/3)    *np.float64(f['/simulation/results/%s/fractalOP_Kurt/mean/error'%q])
    plt.errorbar(1/T,  y, dy, label = L, color=color[i], linestyle = '-',linewidth=2, 
                  marker = '.', markersize=8, capsize=3, mfc = markercolor[i], 
                  mec = markercolor[i], ecolor = markercolor[i], zorder=12-i
                )
    ...
    plt.tick_params(which='minor', direction="in")
    plt.legend()
#    plt.ylabel('fractalOP_BC')
#    plt.yscale('log')



#for i in plt.get_fignums():
#    plt.figure(i).legend(loc=1)
